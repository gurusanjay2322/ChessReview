    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            ...
            if (piece && piece.color === color) {
                ...
                // Get all moves for this piece (including captures)
                try {
...
// Find all pieces of a given color with their squares
...
function isSquareDefended(chess, square, byColor) {
    // Temporarily check from opponent's perspective which squares they attack
    const board = chess.board()
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            const piece = board[r][c]
...
// Find hanging pieces (attacked but not defended) for a given color
function findHangingPieces(chess, color) {
    ...
    for (const piece of myPieces) {
        ...
        // We need to check from the enemy's perspective
        // A piece is hanging if it can be captured and isn't defended
...
// Check if a piece was guarding other pieces before moving
function findLostDefense(beforeFen, playedMove) {
    ...
    // Find what the moved piece was defending
    ...
    for (const piece of friendlyPieces) {
        // Was this piece defended by the moved piece before?
        const wasMoveDefender = isSquareDefended(before, piece.square, movedColor)
        // Is this piece still defended after the move?
        const stillDefended = isSquareDefended(after, piece.square, movedColor)

        if (wasMoveDefender && !stillDefended) {
            // Check if this piece is now attacked by enemy
            try {
                // From the opponent's perspective after our move
...
// Check opponent's best responses for captures and checkmates
function analyzeOpponentThreats(afterFen) {
    ...
    // Find all captures available to opponent
    ...
    // Simple fork detection: a single piece attacking 2+ valuable enemy pieces
    const attacksBy = {}
    for (const m of opponentMoves) {
    ...
    // Check for mate-in-1: can opponent deliver checkmate?
    for (const m of opponentMoves) {
...
// Check if the best move was a tactic
...
/**
 * Generate a chess-specific explanation for a bad move.
 * Uses chess.js to analyze the actual position.
 */
export function generateChessExplanation(positionFen, playedMove, analysis) {
    ...
    // 1. Make the played move and get the resulting position
    let afterFen = null
    try {
    ...
    // 2. Check if the move caused direct problems

    // Check for opponent threats after this move
    ...
    // B) Lost defense — did this move abandon a defended piece?
    ...
    // D) Big captures available to opponent
    ...
    // 3. If nothing specific found, describe based on eval swing
    if (parts.length === 0) {
    ...
    // 4. Describe what the best move does
    ...
    if (bestTactic && bestMoveSan) {
        if (bestTactic.isCapture && bestTactic.isCheck) {
            ...
            // Describe the best move's purpose based on eval
            if (bestScoreCp > 300) {
